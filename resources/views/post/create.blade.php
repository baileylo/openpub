    ...
    <form action="{{ route('post.store') }}" method="POST">
        ...
        <div class="row">
            <div class="large-12 columns @if($errors->has('title')) error @endif">
                <input type="text" id="title" name="title" placeholder="Title: Name of the post" required value="{{ old('title') }}">
                @if($errors->has('title'))
        ...
        <div class="row">
            <div class="large-12 columns @if($errors->has('categories')) error @endif">
                <input type="text" name="categories" value="{{ old('categories') }}" placeholder="Categories: Insert as many as you want separated by a coma" required />
        ...
        <div class="row">
            <div class="large-12 columns @if($errors->has('body')) error @endif">
                <textarea name="body" id="body" cols="30" rows="15" placeholder="Post: Body of the post written in Markdown" required="true">{!! old('body') !!}</textarea>
                @if($errors->has('body'))
